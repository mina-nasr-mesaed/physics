...
class pdf4Student extends StatefulWidget {

  @override
  State<pdf4Student> createState() => _HomeState();
}

class _HomeState extends State<pdf4Student> {
  ...
  void pickFile() async {
    ...
    if(pickedFile != null){
      ...
      _firebaseFirestore.collection("pdfs").doc('TlQZMKeOq6jTUDd9KMgk').collection('level4').add({
        "name":filename,
        "url":downloadlink,
      });
  ...
  void getAllPdf()async{
    final results =await _firebaseFirestore.collection("pdfs").doc('TlQZMKeOq6jTUDd9KMgk').collection('level4').get();
