...
class pdf3Student extends StatefulWidget {

  @override
  State<pdf3Student> createState() => _HomeState();
}

class _HomeState extends State<pdf3Student> {
  ...
  void pickFile() async {
    ...
    if(pickedFile != null){
      ...
      _firebaseFirestore.collection("pdfs").doc('TlQZMKeOq6jTUDd9KMgk').collection('level3').add({
        "name":filename,
        "url":downloadlink,
      });
  ...
  void getAllPdf()async{
    final results =await _firebaseFirestore.collection("pdfs").doc('TlQZMKeOq6jTUDd9KMgk').collection('level3').get();
